;(function(global, factory) {
    ...
    ************************************************************/
    ...
    masterpage.data = {
        employeeList: function (employees, hasCancelItem) {
            var group = groupList(employees, "Department", function (item) {
                return {
                    EmployeeId: item.Department,
                    RealName: item.DepartmentName
                };
            });
            if (hasCancelItem === true) {
                group.splice(0, 0, {
                    EmployeeId: "none",
                    RealName: "取消选择"
                });
        ...
        allEmployeeList: function (employees) {

        }
    };

    function groupList(list, group, groupItemAction) {
        var data = [];
        if (!$.isArray(list)) {
            return data;
        }
